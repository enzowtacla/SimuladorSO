...
class SO:
    ...
    def tratar_req_lock_mutex(self, evento_mutex) -> None:
        ...
        m_id = evento_mutex.mutex_id
        tarefa = self.tarefa_executando_anterior

        if m_id not in self.mutexes:
            self.mutexes[m_id] = Mutex(id=m_id)
        
        mutex = self.mutexes[m_id]

        if mutex.count > 0:
            mutex.count -= 1
        else:
            mutex.tarefas_bloqueadas.append(tarefa)
            tarefa.bloquear()
    ...
    def tratar_req_unlock_mutex(self, evento_mutex) -> None:
        ...
        if m_id in self.mutexes:
            mutex = self.mutexes[m_id]

            tarefa_desbloqueada = mutex.unlock()

            if mutex.tarefas_bloqueadas:
